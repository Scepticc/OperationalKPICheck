...
function safeParse(dateStr: string): Date {
  // parseISO handles YYYY-MM-DD as local date (not UTC)
...
export default function Header({ title, showDatePicker = true }: HeaderProps) {
  ...
  // Sync temp dates when global dates change externally
  useEffect(() => {
            ...
            {showPicker && (
              <div className="absolute right-0 top-full mt-1.5 z-50 bg-white dark:bg-gray-900 rounded-lg shadow-xl border border-gray-200 dark:border-gray-700 p-4 w-[520px]">
                <div className="flex gap-4">
                  {/* Presets */}
                  <div className="w-32 shrink-0 border-r border-gray-100 dark:border-gray-800 pr-4">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex justify-end mt-3 pt-3 border-t border-gray-100 dark:border-gray-800 gap-2">
                      <button onClick={() => setShowPicker(false)} className="btn-secondary py-1.5 px-3 text-xs">
                        Cancel
                      </button>
