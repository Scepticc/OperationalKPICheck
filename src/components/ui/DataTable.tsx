...
export default function DataTable<T extends object>({
  ...
  const fetchData = useCallback(async (pg: number) => {
    ...
    try {
      ...
      const qp = new URLSearchParams({
        page: String(pg), pageSize: String(pageSize), search, sortCol, sortDir, ...extra,
      });
  ...
  function clearDateFilter() { setDateStart(''); setDateEnd(''); setPage(1); }
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className={cn('flex items-center gap-2 text-xs text-gray-500', !dateColumn && 'ml-auto')}>
          <span>Rows:</span>
          <select className="input h-8 py-0 pr-6 text-xs" value={pageSize} onChange={(e) => { setPageSize(parseInt(e.target.value)); setPage(1); }}>
      ...
      <div className="card overflow-hidden">
                ...
                {columns.map((col) => (
                  <th key={String(col.key)} className={cn('table-th', col.width && `w-${col.width}`)} onClick={() => col.sortable !== false && handleSort(String(col.key))}>
                    <div className="flex items-center gap-1">
                      {col.header}
                      {col.sortable !== false && <SortIcon col={String(col.key)} currentCol={sortCol} dir={sortDir} />}
                    </div>
                  </th>
                ))}
              </tr>
              ...
              {state.loading ? (
                ...
                state.data.map((row, i) => (
                  <tr key={((row as Record<string, unknown>).id as string | number) ?? i} className="table-row">
                    {columns.map((col) => (
                      <td key={String(col.key)} className="table-td">{renderCell(row, col)}</td>
                    ))}
                  </tr>
                ))
              )}
        ...
        <div className="flex items-center justify-between px-3 py-2.5 border-t border-gray-200 bg-gray-50/50">
          ...
          <div className="flex items-center gap-0.5">
            ...
            <span className="text-xs text-gray-500 px-2 tabular-nums">{page} / {state.totalPages || 1}</span>
          </div>
        </div>
      </div>
    </div>
